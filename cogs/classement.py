...
ACTION_LABEL = {
    "win":"✅ Victoire","loose":"❌ Défaite",
    "champion":"🥇 Champion","vice":"🥈 Vice-Champion",
    "semi":"🏅 Demi-Finale","quart":"🎖️ Quarts de Finale",
}
POINTS_TABLE = {
    "S":{"scrim":{"win":1,"loose":-5},"tournoi":{"win":2,"loose":-6,"champion":2,"vice":1,"semi":0},"officiel":{"win":5,"loose":-10,"champion":4,"vice":3,"semi":2,"quart":1}},
    "A":{"scrim":{"win":3,"loose":-5},"tournoi":{"win":4,"loose":-6,"champion":3,"vice":2,"semi":1},"officiel":{"win":8,"loose":-6,"champion":5,"vice":4,"semi":3,"quart":2}},
    "B":{"scrim":{"win":3,"loose":-4},"tournoi":{"win":4,"loose":-5,"champion":4,"vice":3,"semi":2},"officiel":{"win":10,"loose":-10,"champion":10,"vice":5,"semi":3,"quart":1}},
    "C":{"scrim":{"win":4,"loose":-3},"tournoi":{"win":4,"loose":-4,"champion":5,"vice":3,"semi":1},"officiel":{"win":10,"loose":-8,"champion":15,"vice":10,"semi":5,"quart":3}},
    "D":{"scrim":{"win":5,"loose":-3},"tournoi":{"win":6,"loose":-4,"champion":10,"vice":5,"semi":3},"officiel":{"win":10,"loose":-6,"champion":20,"vice":15,"semi":10,"quart":5}},
    "E":{"scrim":{"win":5,"loose":-1},"tournoi":{"win":8,"loose":-3,"champion":15,"vice":10,"semi":6},"officiel":{"win":10,"loose":-5,"champion":25,"vice":20,"semi":15,"quart":10}},
}
...
def calc_pts(tier, evt, action):
    return POINTS_TABLE.get(tier,{}).get(evt,{}).get(action, 0)

def build_classement_embed(guild_id, guild):
    ...
    for i, row in enumerate(rows[:25], 1):
        ...
        tier  = row.get("tier","E")
        lines.append(f"{medal} {TIER_EMOJI.get(tier,'⚪')} **{name}** — `{row['points']} pts` · Tier {tier}")
    embed.description = "\n".join(lines)
...
class AttrTeamView(discord.ui.View):
    ...
    async def on_team(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(
            content=f"✅ **{role.name}** — Type d'événement :",
...
class AttrTypeView(discord.ui.View):
    ...
    async def on_type(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(
            content=f"✅ **{labels[evt]}** — Tier de VOTRE équipe :",
...
class AttrTierView(discord.ui.View):
    ...
    async def on_tier(self, interaction: discord.Interaction):
        ...
        if evt == "scrim":
            opts = [discord.SelectOption(label="✅ Victoire (Win)",  value="win"),
                    discord.SelectOption(label="❌ Défaite (Loose)", value="loose")]
        elif evt == "tournoi":
            opts = [discord.SelectOption(label="✅ Win (match)",     value="win"),
                    discord.SelectOption(label="❌ Loose (match)",   value="loose"),
                    discord.SelectOption(label="🥇 Prime Champion",  value="champion"),
                    discord.SelectOption(label="🥈 Prime Vice",      value="vice"),
            ...
            opts = [discord.SelectOption(label="✅ Win (match)",      value="win"),
                    ...
                    discord.SelectOption(label="🥈 Prime Vice",       value="vice"),
                    discord.SelectOption(label="🏅 Prime Demi-Finale",value="semi"),
                    discord.SelectOption(label="🎖️ Prime Quarts",    value="quart")]
        await interaction.response.edit_message(
            content=f"✅ **Tier {tier}** — Résultat / Placement :",
...
class AttrResultView(discord.ui.View):
    def __init__(self, user, opts):
        super().__init__(timeout=120)
        self.user = user
        sel = discord.ui.Select(placeholder="Résultat...", options=opts)
    ...
    async def on_result(self, interaction: discord.Interaction):
        ...
        pts    = calc_pts(sess.get("tier","E"), sess.get("type","scrim"), action)
        reason = f"Tier {sess.get('tier','E')} • {sess.get('type','scrim')} • {ACTION_LABEL.get(action,action)}"
        db.add_points(str(interaction.guild_id), sess["role_id"], pts, reason)

        sign  = "+" if pts >= 0 else ""
        color = 0x23A55A if pts > 0 else (0xC0392B if pts < 0 else 0x808080)
        embed = discord.Embed(title="📊 Points attribués", color=color)
        embed.add_field(name="Team",       value=f"**{sess.get('name','?')}**", inline=True)
        embed.add_field(name="Événement",  value=reason,                        inline=True)
        embed.add_field(name="Points",     value=f"**{sign}{pts}**",            inline=True)
        team = db.get_team(str(interaction.guild_id), sess["role_id"])
        if team:
            embed.add_field(name="Total", value=f"{team['points']} pts", inline=True)
...
class AttribuerView(discord.ui.View):
    ...
    async def attribuer(self, interaction: discord.Interaction, btn: discord.ui.Button):
        if not interaction.user.guild_permissions.manage_guild:
            return await interaction.response.send_message("❌ Admins uniquement.", ephemeral=True)
        await interaction.response.send_message(
            "**Attribution — Sélectionne la team :**",
...
class ClassementCog(commands.Cog):
    ...
    async def setup_attr(self, interaction: discord.Interaction):
        embed = discord.Embed(
            ...
            description="Utilise ce panel pour attribuer des points aux équipes. Réservé aux **admins**.",
            color=0xF0B232)
    ...
    @app_commands.command(name="set-tier", description="Définir le tier d'une équipe")
    ...
    @app_commands.command(name="reset-points",
                          description="[ADMIN] Remet à 0 les points d'une équipe")
    ...
    @app_commands.command(name="export-ligalabs",
                          description="[ADMIN] Génère ligalabs.json et l'envoie ici en fichier")
    @app_commands.checks.has_permissions(administrator=True)
    async def export_ligalabs(self, interaction: discord.Interaction):
        ...
        for m in matches:
            ...
            k1, k2  = f"{t1_name}||{m['roster1']}", f"{t2_name}||{m['roster2']}"
        ...
        for s in stats.values():
            ...
            standings.append({
                "team": s["team"], "roster": s["roster"], "logo": s["logo"],
                "played": s["played"], "required": required,
                "wins": s["wins"], "losses": s["losses"], "pct": pct,
                "qualified": s["played"] == 0 or s["played"] >= seuil60,
            })
        standings.sort(key=lambda x: (-x["wins"], x["losses"]))

        output = {
            "season": "Saison 1", "total_rosters": total,
            "required_per": required, "seuil_60pct": seuil60,
            "standings": standings, "match_count": len(matches),
        }
        ...
        try:
            ...
            with open(teams_path.replace("teams","ligalabs"), "wb") as f:
                f.write(data)
        except Exception:
            pass

        file = discord.File(io.BytesIO(data), filename="ligalabs.json")
        await interaction.followup.send(
            f"✅ **ligalabs.json** — {len(standings)} rosters, {len(matches)} matchs\n\n"
            "**Pour mettre à jour le site :**\n"
            "1. Télécharge ce fichier\n"
            "2. Remplace `website/ligalabs.json` dans ton dossier\n"
            "3. GitHub Desktop → Commit → Push",
            file=file, ephemeral=True)

    @app_commands.command(name="export-site",
                          description="[ADMIN] Génère teams_points.json avec les points actuels")
    @app_commands.checks.has_permissions(administrator=True)
    async def export_site(self, interaction: discord.Interaction):
        ...
        # Charger teams.json existant pour conserver toutes les infos
        existing = {}
        teams_path = "website/teams.json"
        if os.path.exists(teams_path):
            try:
        ...
        for row in rows:
            ...
            base = existing.get(row["role_id"], existing.get(name, {}))
            teams.append({
                "role_id":     row["role_id"],
                ...
                "tier":        row.get("tier", "E"),
                "logo":        base.get("logo", ""),
                "annee":       base.get("annee", ""),
                "nationalite": base.get("nationalite", "🇫🇷 France"),
                "rosters":     base.get("rosters", []),
            })

        data = json.dumps({"teams": teams}, indent=2, ensure_ascii=False).encode("utf-8")
        ...
        file = discord.File(io.BytesIO(data), filename="teams.json")
        await interaction.followup.send(
            f"✅ **teams.json** — {len(teams)} équipes\n\n"
            "**Pour mettre à jour le site :**\n"
            "1. Télécharge ce fichier\n"
            "2. Remplace `website/teams.json` dans ton dossier\n"
            "3. GitHub Desktop → Commit → Push",
            file=file, ephemeral=True)
